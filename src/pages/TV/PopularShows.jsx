...
import { useGetPopularShowsQuery } from '../../slices/tvApiSlice';
...
const PopularShows = () => {
  ...
  const { data, loading: isPopularTvshowsLoading, error: populartvshowsError, refetch } = useGetPopularShowsQuery(pagenum, {
    onSuccess: () => {
  ...
  const handleScroll = useCallback(() => {
    if(isPopularTvshowsLoading) return;

    const isBottom = window.innerHeight + window.scrollY >= document.body.offsetHeight - 200;
    
    if (isBottom && !isPopularTvshowsLoading && data?.total_pages > pagenum) {
      setPagenum((prevPageNum) => prevPageNum + 1);
      console.log("pagechange", pagenum)
    }
  }, [data, pagenum, isPopularTvshowsLoading]);
  ...
  useEffect(() => {
    if (!isPopularTvshowsLoading && data) {
