...
import { useGetAiringTodayShowsQuery } from '../../slices/tvApiSlice';
...
const AiringtodayShows = () => {
  ...
  const { data, loading: isAiringtodayTvshowsLoading, error: airingtodaytvshowsError, refetch } = useGetAiringTodayShowsQuery(pagenum, {
    onSuccess: () => {
  ...
  const handleScroll = useCallback(() => {
    if(isAiringtodayTvshowsLoading) return;

    const isBottom = window.innerHeight + window.scrollY >= document.body.offsetHeight - 200;
    
    if (isBottom && !isAiringtodayTvshowsLoading && data?.total_pages > pagenum) {
      setPagenum((prevPageNum) => prevPageNum + 1);
      console.log("pagechange", pagenum)
    }
  }, [data, pagenum, isAiringtodayTvshowsLoading]);
  ...
  useEffect(() => {
    if (!isAiringtodayTvshowsLoading && data) {
  ...
  }, [isAiringtodayTvshowsLoading, data]);
